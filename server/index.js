...
Sentry.init({
  dsn: "https://f8852dc223a1492c9f2078faa93dbcd2@o769978.ingest.sentry.io/5795238",
  integrations: [
...
io.on("connection", function (client) {
  ...
  client.on("join", function (data) {
    pool.query(
      "select * from devorders where order_id in (SELECT order_id FROM devorders order BY order_id DESC LIMIT 20) AND isclosed = false ORDER BY order_id asc;",
      (err, res) => {
...
var weatherAPI = {
  url:
    "https://api.openweathermap.org/data/2.5/weather?q=Luton,uk&appid=ccda9d309ff5322478451b54ef0cfa38&units=metric",
  method: "GET",
  timeout: 0,
  headers: {
...
function doesOrderContainFoyer(orderData) {
  if (orderData != null) {
    var itemsInOrder = orderData.length;
  }
  var count = -1;
...
setInterval(function () {
  ...
  pool.query(
    "SELECT * FROM devorders order BY order_id DESC LIMIT 20;",
    (err, res) => {
      io.sockets.emit("cache", {
        db: res.rows,
      });
    }
  );
...
function timeDiffStr(createdTime) {
  ...
  const timeopenStr =
    timeopen.getMinutes() + "m " + timeopen.getSeconds() + "s";
  return timeopenStr;
}
...
setInterval(function () {
  ...
  request(options, function (error, response) {
    ...
    request(options1, function (error, response) {
      ...
      //send to pg
      var thisQuery =
        "INSERT INTO public.devorders (order_id, products, istable, isnew, isclosed, isprocessing, time, tablenum, isfoyer, timeopen) VALUES (" +
        auth1.purchases[0].globalPurchaseNumber +
        ", '" +
        JSON.stringify(auth1.purchases[0].products) +
        "'," +
        ...
        theTime +
        ",'" +
        getTableNum(auth1.purchases[0].products) +
        "', '" +
      ...
      pool.query(maxQ, (err, res) => {
        ...
        if (newOrder == true) {
          ...
          //alert over socket
          nextVal = thisVal + 1;
          if (nextVal == auth1.purchases[0].globalPurchaseNumber)
            //io.sockets.emit('broadcast',{ description: true});
...
app.post("/update", (req, res) => {
  ...
  res.send(
    "Order:" +
      id +
      " has been updated at the column " +
      column +
      " with the value: " +
      value
  );
...
app.post("/setStats", (req, res) => {
  ...
  var thisQuery =
    "INSERT INTO .stats (date, avgtime, diff) VALUES ('" +
    date +
    "', " +
    avg +
    ", " +
